...
void queue_init(queue_t *q, size_t s) {
  /* We're going to use a trick where we mmap two adjacent pages (in virtual
   * memory) that point to the same physical memory. This lets us optimize
   * memory access, by virtue of the fact that we don't need to even worry about
   * wrapping our pointers around until we go through the entire buffer. Too bad
   * this isn't portable, because it's so fun.
   */
  ...
  if ((q->buffer = mmap(NULL, 2 * s, PROT_NONE, MAP_PRIVATE | MAP_ANONYMOUS, -1,
                        0)) == MAP_FAILED) {
    queue_error_errno("Could not allocate virtual memory");
  }

  // Mmap first region
  if (mmap(q->buffer, s, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_FIXED, q->fd,
           0) == MAP_FAILED) {
